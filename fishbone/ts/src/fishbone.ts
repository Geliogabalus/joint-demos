...
interface FishboneLayoutOptions {
    top?: boolean; // true for top, false for bottom
    vsGap?: number; // vertical gap between nodes
    vcGap?: number; // vertical gap between nodes and the tail
    hsGap?: number; // horizontal gap between nodes
    hcGap?: number; // gap between the first node and the tail
}

export function layoutFishbone(graph: dia.Graph, x: number, y: number, options: FishboneLayoutOptions = {}) {
    ...
    const topChildren: dia.Element[] = [];
    const bottomChildren: dia.Element[] = [];
...
function getChildren(graph: dia.Graph, node: dia.Element) {
    return util.sortBy(graph.getNeighbors(node, { outbound: true }), (child) => {
        return child.get('order') || 0;
    });
}

function measureNode(graph: dia.Graph, node: dia.Element, children: dia.Element[], horizontal: boolean, level = 0, last: boolean, options: FishboneLayoutOptions = {}) {
...
function positionNode(graph: dia.Graph, node: dia.Element, horizontal: boolean, x: number, y: number, level = 0, options: FishboneLayoutOptions = {}) {
...
function positionVerticalChildren(
    graph: dia.Graph,
...
function positionHorizontalChildren(
    graph: dia.Graph,
    children: dia.Element[],
    node: dia.Element,
    x: number,
