...
export const init = () => {
    ...
    let reqAnimId: null | number = null;
    ...
    keyboard.on('space', (evt: dia.Event) => {
        evt.preventDefault();
        startButton.focus();
        if (reqAnimId) {
            startButton.removeAttribute('disabled');
    ...
    function buildGraph(dataset: IData) {
        const elements: Array<Node | Link> = [];
    ...
    function createLayout(graph: dia.Graph, data: IData) {
        ...
        DirectedGraph.layout(graph, {
            ...
            edgeSep: 5,
            align: 'UR',
            setVertices: function(link: dia.Link, vertices: Array<g.PlainPoint>) {
    ...
    function setTokens() {
        ...
        const tokenElements: Array<Token> = [];

        // Token colors - Each event id in ascending order has its own color
        const eventNumbers: Array<number> = [];
        const eventColors: Array<string> = [
            '#FFFFFF',
            '#EEF0FB',
            '#DEE2F7',
            '#CDD3F3',
        ...
        tokenLinks.forEach((tokenLink) => {
            tokensData.listOfEvent.forEach((tokenEvent) => {
                ...
                if ((tokenEvent.complete > tokenEvent.start) && condition) {
                    ...
                    // Get token color
                    const eventColorIndex: number = eventNumbers.indexOf(Number(tokenEvent.eventId));
                    const tokenColor: string = (eventColorIndex === -1) ? '#394FD0' : eventColors[eventColorIndex];
                    ...
                    const allPoints: Array<{ x: number; y: number; rotation: number }> = [];
    ...
    new ui.Tooltip({
        ...
        content: function(element: Element): string {
            const { model: { attributes }} = paper.findView(<SVGElement>element);
