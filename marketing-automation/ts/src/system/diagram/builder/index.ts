 ...
 */
export function buildDiagram(data: SystemDiagramJSON, graph: dia.Graph, options: BuildDiagramOptions = {}) {
    const {
 ...
 */
function updateGraph(graph: dia.Graph, json: SystemDiagramJSON, buildNode: BuildNode, growthLimit: GrowthLimit) {

    const nodes: dia.Graph.CellInit[] = [];
    const edges: dia.Graph.CellInit[] = [];

    const growthLimits = new Map<dia.Cell.ID, number>();
    const nodeIds = extractNodesIds(json);
    const addedNodeIds = new Set<dia.Cell.ID>();
    const addedButtonIds = new Set<dia.Cell.ID>();
    const replacedNodePositions = new Map<dia.Cell.ID, dia.Point>();
    ...
    nodeIds.forEach(sourceId => {
        ...
        if (!nodeType) {
            ...
            node = buildNode(nodeJSON as SystemTypedNodeData, sourceId.toString());
            ...
            growthLimits.set(sourceId, growthLimit(nodeJSON as SystemTypedNodeData));
        }
 ...
 */
function setInitialPositionsForAnimation(
    graph: dia.Graph,
    json: SystemDiagramJSON,
    addedNodeIds: Set<dia.Cell.ID>,
    addedButtonIds: Set<dia.Cell.ID>,
    replacedNodePositions: Map<dia.Cell.ID, dia.Point>
) {
    const nodeIds = extractNodesIds(json);

    // Build parent-child relationship map
    const parentMap = new Map<dia.Cell.ID, dia.Cell.ID>();
    nodeIds.forEach(sourceId => {
        const nodeJSON = json[sourceId];
     ...
     */
    const positionsMap = new Map<dia.Cell.ID, dia.Point>();

    // Capture existing node positions
    nodeIds.forEach(sourceId => {
        const nodeModel = graph.getCell(sourceId) as dia.Element;
    ...
    addedNodeIds.forEach(nodeId => {
        const nodeModel = graph.getCell(nodeId) as dia.Element;
        if (!nodeModel) return;
    ...
    addedButtonIds.forEach(buttonId => {
        const button = graph.getCell(buttonId) as dia.Element;
        if (button) {
...
function constructButtonId(nodeId: dia.Cell.ID): dia.Cell.ID {
 ...
 */
function retrieveParentPositionRecursively(nodeId: dia.Cell.ID, parentMap: Map<dia.Cell.ID, dia.Cell.ID>, positions: Map<dia.Cell.ID, dia.Point>): dia.Point {
    ...
    if (!parentPosition) return retrieveParentPositionRecursively(parentId, parentMap, positions);

    // The parent has defined position therefore it's already part of the graph, return it
    return parentPosition;
}

function makeButtons(nodeIds: dia.Cell.ID[], data: SystemDiagramJSON, growthLimits: Map<dia.Cell.ID, number>) {

    const cells: dia.Graph.CellInit[] = [];

    nodeIds.forEach(nodeId => {
        const nodeJSON = data[nodeId];
        if (!nodeJSON || !nodeJSON.type) return;
...
function makeButton(nodeId: dia.Cell.ID) {
    const buttonId = constructButtonId(nodeId);
    const buttonLineId = `${buttonId}-line`;

    const buttonInit: dia.Graph.CellInit = {
        id: buttonId,
