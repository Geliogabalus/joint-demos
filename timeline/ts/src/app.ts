...
export const init = () => {
    ...
    const stencil = new ui.Stencil({
        ...
        dragStartClone: (element) => {
            ...
            let clone = element.clone() as dia.Element;
    ...
    const TreeLayoutView: typeof ui.TreeLayoutView = ui.TreeLayoutView.extend({
        updateParentPreview: function(position: dia.Point, size: dia.Size, parent: dia.Element) {
    ...
    const tree = new layout.TreeLayout({
        ...
        filter: (children, parent) => {
            ...
            return artificialIntelligenceTimeline.getBusElements().map((id) => graph.getCell(id) as dia.Element);
        }
    ...
    const treeView = new TreeLayoutView({
        ...
        validateConnection: (element, candidate) => {
            ...
            return (candidate as ITimelineShape).isConnectionValid(element.get('type'));
        },
    ...
    graph.on('change:attrs', (element, attrs, { propertyPath }) => {
        if (!(element instanceof Category) || propertyPath !== 'attrs/label/text') return;
        element.updateSize();
        layoutDiagram(tree, paperScroller, selection);
    });
    ...
    paper.on('edit', (elementView: dia.ElementView, evt: dia.Event) => {

        // Only allow double click to edit
        if ((evt.originalEvent as MouseEvent).detail !== 2) return;

        evt.stopImmediatePropagation();

        const element = elementView.model as ITimelineShape;
        ...
        dialog.on('action:save', () => {
            const inputs = content.querySelectorAll('input, textarea') as NodeListOf<HTMLTextAreaElement | HTMLInputElement>;
        ...
        dialog.open();
        (content.firstChild as HTMLTextAreaElement | HTMLInputElement).focus();
    });
    ...
    keyboard.on('delete backspace shift+delete shift+backspace', (evt) => {
        ...
        else if (selected.some((element) => element.get('busElement')) && artificialIntelligenceTimeline.getBusElements().length === 1) return;
