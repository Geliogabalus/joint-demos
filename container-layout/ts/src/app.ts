...
export const init = () => {
    ...
    paper.on('element:pointermove', (view: dia.ElementView, evt: dia.Event, x: number, y: number): void => {
...
const rescale = (scroller: ui.PaperScroller): void => {
...
const gridPathData = (metrics: layout.GridLayout.GridMetrics, offset: dia.PaddingJSON, padding: number): string => {
    ...
    metrics.gridX.forEach((gx, index, gridX: number[]) => {
    ...
    metrics.gridY.forEach((gy, index, gridY: number[]) => {
        // Skip the first and last line
...
const layoutCells = (graph: dia.Graph): void => {
    ...
    DirectedGraph.layout(directedGraphCells, {
        setPosition: (el: dia.Element, center: dia.BBox): void => {
            const size = el.size();
